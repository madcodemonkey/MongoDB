...
[TestClass]
public class WebHostBuilder_PersonController_ReplacePerson_Method_Tests
{
    [TestMethod]
    public async Task ReplacePerson_UserIsUpdated_WhenAgeIsChanged()
    {
        ...
        var existingPerson = await CreateExistingPersonAsync(client, "John", "Doe", 34);
        ...
        // Act
        var result = await client.PutAsync($"/person/{existingPerson.Id}", context);
        ...
        string newFirstName, string newLastName)
    {
        ...
        var existingPerson = await CreateExistingPersonAsync(client, currentFirstName, currentLastName, 42);
        ...
        string person2NewFirstName, string person2NewLastName)
    {
        ...
        var existingPerson1 = await CreateExistingPersonAsync(client, person1FirstName, person1LastName, 34);
        var existingPerson2 = await CreateExistingPersonAsync(client, person2FirstName, person2LastName, 32);
        ...
        // Act
        var result = await client.PutAsync($"/person/{existingPerson2.Id}", context);
    ...
    /// </summary>
    private async Task<PersonResponse> CreateExistingPersonAsync(HttpClient client, string firstName, string lastName, int age)
    {
        var context = JsonContent.Create(new PersonRequest
        {
            FirstName = firstName,
